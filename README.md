...
Private Vercel functions for authenticated browser AI/push requests and QStash reminder schedules.
...
The Telegram bot was removed in February 2026 (unused). Its `TELEGRAM_*`
environment variables can be deleted from Vercel, its QStash schedules
(bodies marked `__telegram: true`) removed in the Upstash console, and the
bot webhook dropped via `https://api.telegram.org/bot<token>/deleteWebhook`.
...
- Cron endpoints fail closed when their secret is absent.
...
2. Confirm the deployed Firestore rule contains the real UID, not the repository placeholder.
3. Smoke-test authenticated AI, push subscription, and one scheduled callback.
...
Browser writes compare the revision they loaded before updating a domain. A
mismatch never overwrites newer data: the browser asks which version to keep.
Deletions use tombstones so legacy values cannot reappear during migration.
